...
TFile *pippo  = new TFile("covarianceMatrixMinuitProfNewModel.root","RECREATE");
...
ldfInfos getData() {
	ldfInfos p;
	TFile *a = new TFile("histogramsLDFRopt300.root");//250m
	TFile *a100 = new TFile("histogramsLDFRopt300.root");//250m
...
double fmodel(double *x, double *par)
{
	...
	double y = a0 + a1*lgSRef + sectheta*(b0 + b1*lgSRef)+ pow(sectheta,2)*(c0 + c1*lgSRef + c2*lgSRef*lgSRef);
...
double fitchi2(const double *fitpar) {
	...
	double par6 = fitpar[6];
	TF2 fitf("fitf",fmodel, 1, 1.4, 0.6, 2.2, 7); //per Ropt300 ho bin logSref[0.6,2.2] mentre per Ropt250 ho tra Sropt[1,2.6]
...
void minuit2TprofileNew() {
	std::fstream outFile;
	outFile.open("fitParMinProf7par.txt", outFile.out);
	...
	min->SetVariable(0,"par0", par[0], 0.01);
        ...
        min->SetVariable(6,"par6", par[6], 0.01);
...
/*	min->SetVariableLimits(0,par[0]-3*parErr[0],par[0]+3*parErr[0]);
	...
	double etahat     = parameters.Value(6);	
